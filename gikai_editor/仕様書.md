...
- **第2部 ソースコード** — 全 40 ファイル・約 13,338 行の中身
...
前号の様式に差し込む方式。枠を見つけて中身だけ入れ替える。（550 行）
...
画面の中の「使い方」。文章はここ1か所にまとめてある。（244 行）
...
SECTIONS: list[dict] = [
    ...
    },
    {
        ...
        "blocks": [
            ...
            {"note": "議会だよりは印刷の都合で偶数ページのことが多いので、"
                     "2の倍数（8・12・16 など）を入れておくと扱いやすくなります。"},
...
かんたん作成。区分ごとのフォルダの中身から、1号ぶんを組み立てる。（643 行）
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（185 行）
...
全体のテスト。pytest が無くても直接実行できる。（1973 行）
