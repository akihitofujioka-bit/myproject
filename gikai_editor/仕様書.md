...
- **第2部 ソースコード** — 全 36 ファイル・約 10,007 行の中身
...
自動組版。5段縦書きの Word を組み立て、ページ数を見積もる。（427 行）
...
1号ぶんの作業データ。記事・写真・設定の保存と読み出し。（579 行）
...
ローカル専用サーバ。画面とのやりとりはすべてここを通る。（553 行）
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（130 行）
...
この仕様書を作り直すとき。第1部の文章もこの中に入っているため、以下のコードには仕様の文言がそのまま現れる。（646 行）
...
全体のテスト。pytest が無くても直接実行できる。（1284 行）
