...
- **第2部 ソースコード** — 全 41 ファイル・約 14,494 行の中身
...
docx / doc / txt / pdf / rtf の取り込み。文字コードの自動判別。（475 行）
...
Excel(.xlsx)と CSV を、行と列のまま読む。賛否一覧表のため。（178 行）
...
自動組版。5段縦書きの Word を組み立て、ページ数を見積もる。（628 行）
...
かんたん作成。区分ごとのフォルダの中身から、1号ぶんを組み立てる。（863 行）
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（219 行）
...
全体のテスト。pytest が無くても直接実行できる。（2342 行）
