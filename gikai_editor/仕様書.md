...
- **第2部 ソースコード** — 全 41 ファイル・約 15,041 行の中身
...
自動組版。5段縦書きの Word を組み立て、ページ数を見積もる。（812 行）
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（261 行）
...
全体のテスト。pytest が無くても直接実行できる。（2573 行）
