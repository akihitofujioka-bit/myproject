...
- **第2部 ソースコード** — 全 40 ファイル・約 13,700 行の中身
...
画面の中の「使い方」。文章はここ1か所にまとめてある。（272 行）
...
かんたん作成。区分ごとのフォルダの中身から、1号ぶんを組み立てる。（778 行）
...
議会だよりの構成（表紙→行政報告→…→最終ページ）。原稿がどの区分のものかの判定もここ。（204 行）
...
ローカル専用サーバ。画面とのやりとりはすべてここを通る。（743 行）
...
<section id="pane-easy" class="pane on">
  ...
  <div class="card ez" id="ezStep1">
    ...
    <div class="row" style="margin-top:12px">
      <label>最大ページ数
        <input id="ezPages" type="number" min="0" max="64" step="2" value="8"></label>
      <button id="ezSavePages">この枚数にする</button>
      <span class="hint" id="ezPagesNote">この枚数に収まるように、長い原稿を自動で要約します。
        0 なら成り行き（原稿の分量どおり）。</span>
    </div>
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（202 行）
...
全体のテスト。pytest が無くても直接実行できる。（2108 行）
