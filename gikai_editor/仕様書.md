...
- **第2部 ソースコード** — 全 40 ファイル・約 13,796 行の中身
...
docx / doc / txt / pdf / rtf の取り込み。文字コードの自動判別。（459 行）
...
自動組版。5段縦書きの Word を組み立て、ページ数を見積もる。（527 行）
...
画面の中の「使い方」。文章はここ1か所にまとめてある。（279 行）
...
かんたん作成。区分ごとのフォルダの中身から、1号ぶんを組み立てる。（802 行）
...
1号ぶんの作業データ。記事・写真・設定の保存と読み出し。（717 行）
...
ローカル専用サーバ。画面とのやりとりはすべてここを通る。（745 行）
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（213 行）
...
この仕様書を作り直すとき。第1部の文章もこの中に入っているため、以下のコードには仕様の文言がそのまま現れる。（672 行）
...
全体のテスト。pytest が無くても直接実行できる。（2144 行）
