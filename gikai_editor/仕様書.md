...
- **第2部 ソースコード** — 全 39 ファイル・約 12,103 行の中身
...
かんたん作成。区分ごとのフォルダの中身から、1号ぶんを組み立てる。（333 行）
...
ローカル専用サーバ。画面とのやりとりはすべてここを通る。（688 行）
...
.fold .flist{font-size:11px;color:var(--sub);margin-top:4px;line-height:1.5;
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（161 行）
...
この仕様書を作り直すとき。第1部の文章もこの中に入っているため、以下のコードには仕様の文言がそのまま現れる。（664 行）
...
全体のテスト。pytest が無くても直接実行できる。（1671 行）
