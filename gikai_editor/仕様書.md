...
- **第2部 ソースコード** — 全 41 ファイル・約 14,727 行の中身
...
自動組版。5段縦書きの Word を組み立て、ページ数を見積もる。（673 行）
...
画面の中の「使い方」。文章はここ1か所にまとめてある。（315 行）
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（244 行）
...
**まだ実物に届いていない点**（実物 18ページ、ツール 10ページ）:
柱（ページ上部の号数・紙名・日付・ページ番号）、右端の区分タブ、
全面写真の表紙、質問／答弁の色分け、記事ごとに変わる段数、写真の大きさ。
...
全体のテスト。pytest が無くても直接実行できる。（2440 行）
