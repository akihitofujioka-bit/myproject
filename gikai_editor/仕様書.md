...
- **第2部 ソースコード** — 全 37 ファイル・約 10,858 行の中身
...
docx / doc / txt / pdf / rtf の取り込み。文字コードの自動判別。（429 行）
...
自動組版。5段縦書きの Word を組み立て、ページ数を見積もる。（466 行）
...
前号の様式に差し込む方式。枠を見つけて中身だけ入れ替える。（546 行）
...
1号ぶんの作業データ。記事・写真・設定の保存と読み出し。（712 行）
...
ローカル専用サーバ。画面とのやりとりはすべてここを通る。（575 行）
...
.steps{display:flex;gap:6px;padding:10px 18px;background:#fff;
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（142 行）
...
この仕様書を作り直すとき。第1部の文章もこの中に入っているため、以下のコードには仕様の文言がそのまま現れる。（654 行）
...
全体のテスト。pytest が無くても直接実行できる。（1438 行）
