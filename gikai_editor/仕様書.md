...
- **第2部 ソースコード** — 全 41 ファイル・約 15,933 行の中身
...
自動組版。5段縦書きの Word を組み立て、ページ数を見積もる。（1306 行）
...
前号の様式に差し込む方式。枠を見つけて中身だけ入れ替える。（604 行）
...
画面の中の「使い方」。文章はここ1か所にまとめてある。（394 行）
...
かんたん作成。区分ごとのフォルダの中身から、1号ぶんを組み立てる。（889 行）
...
ローカル専用サーバ。画面とのやりとりはすべてここを通る。（808 行）
...
/* ---------------------------------------------- できあがりのプレビュー */
/* PDF を埋め込むのではなく、1ページずつ画にして並べる。
   パソコンの設定に左右されず、必ず見えるようにするため */
.pvpages{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));
  gap:16px;margin-top:12px}
.pvpage{margin:0;background:#e9ecf1;border:1px solid var(--line);border-radius:8px;
  overflow:hidden}
.pvpage img{width:100%;display:block;background:#fff}
.pvpage figcaption{padding:6px 10px;font-size:12px;color:var(--sub);background:#fff;
  border-top:1px solid var(--line);text-align:center}
```

### `gikai/static/app.js`

画面の動き。通信先はこのパソコンの中だけ。（1885 行）
...
コードをさわる人・AI 向けの約束ごと。踏んではいけない罠の一覧。（290 行）
...
| **縦組みに横組みを混ぜる** | 方法は**テキストボックス（`_textbox`）だけ**。①区間ごとに `w:textDirection` を切り替える → LibreOffice は文書の最初の向きを全ページに当ててしまい、Word とプレビューで食い違う。②紙のふちからの寸法で図形を置く（`wp:anchor` / `w:framePr`）→ 縦組みでは座標の軸が90度回るので、どちらのソフトでも同じになる保証がない | `test_horizontal_heading_is_a_textbox_and_marker_is_not_printed` |
| **テキストボックスの大きさ** | 箱の**幅**は紙面の横方向（行が並ぶ方向）に取れるが、**高さは段の高さまで**。段の高さを超える箱を5段の区間に置くと、中の行が重なって刷られる。紙面いっぱいの箱（表紙）は**1段の区間**に入れること | `test_cover_is_its_own_page_without_a_running_head` |
| **箱の中の表は箱より狭く** | テキストボックスの中に、箱と同じ幅の表を置かない。入り切らず、中の字が早く折り返して行が重なる。数 mm 狭くする | 同上 |
| **箱の中に箱は入らない** | テキストボックスの中にテキストボックスを置いても描かれない（Word も LibreOffice も）。地色を敷きたいときは**表のセル**にする | 同上 |
| **升目の中もグリッドから外す** | 表のセルの段落にも `snapToGrid=0` を付ける。付けないと本文（10.5pt）向きのグリッドに9ptの字が1マスずつ吸着し、1行に入る字数が半分になって**字が切れて消える** | `test_vote_table_reads_the_same_way_as_the_real_paper` |
| **改ページは `pageBreakBefore`** | `<w:br w:type="page"/>` は、ページが変わった直後にもう一度使うと読み飛ばされる（一般質問で議員が3人続くと2人目と3人目が同じページに出た）。`<w:pageBreakBefore/>` を使う | `test_ippan_gets_one_page_per_member` |
| **`normAutofit` と `wrap="none"`** | テキストボックスに付けない。**最後の1字が1つ前の字に重ねて置かれた**。箱を字数より広めに取り、念のため見出しの末尾に空き1字を足してある | 同上 |
...
- **賛否表は実物と同じ向き**にしてある。縦組みの紙面では表が90度倒れる
  ので、**転置して行を反転**してから組む（`_table_block`）。区間ごとに
  横組みへ切り替える手は LibreOffice が読まないので使えない
  （`test_vote_table_reads_the_same_way_as_the_real_paper`）
- **右端に区分のタブ**を立ててある（`_section_tab`）。区分の頭のページは
  帯があるので出さず、続きのページ（一般質問の2人目以降）に出す
- **表紙はまるごと横組み**（`_cover_page`）。紙面いっぱいのテキスト
  ボックス1つを**1段の区間**に置き、その中を上から順に組む。柱は出さない。
  題字と発行元（`PUBLISHER`）は号によらず決まっているので原稿に要らない
- **一般質問は議員1人につき1ページ**。ページ数の下限
  （`easy.page_floor`）も人数を数える（`test_ippan_gets_one_page_per_member`）
- **見出しの向きは原稿で決める**。行の頭に `【横】` と書くと横組みの帯に
  なる（`heading_dir()` / `_heading_box()`）。実物は一般質問の大見出しが
  横組み、特集などが縦組みで混ざっている
- **数字は縦中横**（`_runs()`）。3文字までの半角数字・英字を横並びのまま
  1文字ぶんに収める。4文字以上は升目からはみ出すので寝かせる。
  **LibreOffice はこの指定を読まない**ので、プレビューでは寝て見える
- **質問は下地あり、答弁は白地**（`qa_kind()` / `_qa_para()`）。頭の
  「質問」「答弁」も濃い青の太字にする。区切り（全角空き・コロン）が
  無いものは目印にしない — 「問題は」「答申を」を拾ってしまうため

**まだ実物に届いていない点**: 区分の頭の大きな横組みの帯（紙面の幅
いっぱいの帯は縦組みの流れと衝突し、本文が回り込めずページがすかすかに
なる。いまは縦の帯）、記事ごとに変わる段数、表紙の題字のロゴ。
...
突き合わせて補正してある**（一般質問0〜10人 × 特集あり／なしの12通りで
実測と一致。`tools` ではなく開発時の実測用スクリプトで測る）。
...
全体のテスト。pytest が無くても直接実行できる。（2758 行）
