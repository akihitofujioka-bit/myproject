...
console.log("== apps/index.html（トップページ）==");
{
  ...
  ok((await lpage.locator("a.app").count()) === 3, "3つのアプリへのリンクがある");
  const hrefs = await lpage.locator("a.app").evaluateAll((els) => els.map((e) => e.getAttribute("href")));
  ok(hrefs.join(",") === "fridge/,docs-tracker/,messenger/", "リンク先が相対パス（公開後もそのまま動く）");
  ok((await lpage.locator("a.app img").count()) === 3, "アイコンが表示される");
  const iconsOk = await lpage.locator("a.app img").evaluateAll((els) => els.every((el) => el.naturalWidth > 0));
  ok(iconsOk, "アイコン画像が実際に読み込める");
