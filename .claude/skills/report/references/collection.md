...
複数のデバイスで同じ会議を録音していることがある（`serial_number` が異なる）。判定は**開始時刻の近さではなく、録音区間の重なりで行う**。片方が遅れて録音を開始した場合、開始時刻は大きく離れていても同じ会議だからである。

1. 各録音を区間 `[start, end]` にする。`start` は `start_at` をJSTに直したもの、`end` は `start + duration`（`duration` はミリ秒）
2. 2件の重なりを `overlap = min(end_a, end_b) - max(start_a, start_b)` で求める。0以下なら重なっていない＝別の会議
3. `overlap` が**短い方の録音時間の50%以上**なら同じ会議とみなす。連続した別の会議が境界で少しだけ接しているケースを誤って結合しないための下限
4. 3件以上が絡む場合は、重なると判定した組を順につないで1つのまとまりにする（A-B と B-C が重なるなら A・B・C で1件）
5. まとまりは1件に統合する。文字起こしがある方を採用し、両方にあれば長い方。時刻は最も早い `start` と最も遅い `end` を取る
